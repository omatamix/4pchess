...
std::optional<std::tuple<int, std::optional<Move>>> AlphaBetaPlayer::Search(
  ...
  if (do_move_level_pruning)
  {
    ...
    if (prior_reduction >= 2 && depth >= 2 && ss->static_eval + (ss - 1)->static_eval > 350) depth--;
  ...
  while (true) {
    ...
    int q = 1 + 
      ((depth * depth) / (improving ? 1 : 2)) + (is_pv_node ? depth : 0);
    ...
    // decrease reduction if our opponent is worsening
